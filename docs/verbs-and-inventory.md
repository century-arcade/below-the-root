...
| 11 vine rope | scan the row `player_row + 1` in the facing direction for the first non-empty cell; nothing before the room edge -> "THE ROPE IS USELESS HERE".  Otherwise fill that span with tile `$E0`, colour 8, and consume the rope |
