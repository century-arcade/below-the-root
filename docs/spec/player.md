...
6. No direction, standing on a doorway, with a fresh button press: go
   through the door. The session adds `press` to each stick sample when
   `fire` was false on the previous read and is true now. Demo end,
   doorway transit and modern-display tune skip share this edge; holding
   the button cannot repeat them. Demo scripts derive the same edge.
