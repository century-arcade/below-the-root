...
import { noteRhythm, QUARTER_FRAMES } from './music-notation.js';
...
export function planTune(music, n) {
  ...
  const measureFrames = 4 * QUARTER_FRAMES[n];
  ...
  tune.voices.forEach((events, voice) => {
    events.forEach((e, i) => {
      ...
      out.push({ voice, midi: note.midi, hz: note.hz_ntsc, start: e.t, stop: Math.min(e.t + decay, next ? next.t : Infinity),
        rhythm: noteRhythm(n, e.dur), measureFrames, measureOffset: e.t % measureFrames });
    });
  });
  return out;
}
...
export class Speaker {
  ...
  notationTime() {
    ...
    // A fixed lag keeps travel speed constant across every rhythm.
    return now - 0.3;
  }
  ...
  recentMeasures(seconds, lookahead = 0) {
    if (!this.ready || !this.playing) return [];
    const { tune, t0 } = this.playing;
    const duration = 4 * QUARTER_FRAMES[tune] * TICK_S;
    const now = this.ctx.currentTime;
    const first = Math.max(0, Math.floor((now - seconds - t0) / duration) + 1);
    const last = Math.floor((Math.min(now + lookahead, this.tuneEnd) - t0) / duration);
    const measures = [];
