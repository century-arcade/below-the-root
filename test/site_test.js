...
assert.equal(startPage('', '#resources', false), 'resources');
assert.equal(startPage('', '#resources', true), 'resources');
...
for (const param of ['demo', 'room', 'player', 'menu', 'debug', 'github']) {
  for (const search of [`?${param}`, `?${param}=T1`, `?unrelated=1&${param}=`]) {
    ...
    assert.equal(startPage(search, '#resources', true), 'resources', 'explicit Resources wins');
  }
}
