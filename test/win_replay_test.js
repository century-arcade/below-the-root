...
import { Session, checkpoint, Autosave, convertRecording } from '../src/record.js';
...
const v1 = JSON.parse(readFileSync(new URL('./fixtures/pomma-win.v1.json', import.meta.url)));
let timing;
assert.deepEqual(convertRecording(data, v1, { onTiming: value => { timing = value; } }), record,
  'the retained v1 fixture converts to the checked-in v2 recording');
assert.equal(Math.floor(timing.old / 1000), 1463);
assert.equal(Math.floor(timing.new / 1000), 1461, 'action-to-read gaps do not belong to any v2 window');
assert.deepEqual(checkpoint(Session.replay(data, { read: () => J.idle }, v1).state), record.checkpoint,
  'v1 autosaves and dropped files use the converter before v2 replay');
const badV1 = structuredClone(v1); badV1.checkpoint.player.food++;
assert.throws(() => convertRecording(data, badV1), /does not replay/);
let joy = J.idle;
const live = { read: () => joy };
...
assert.equal(playTime(replay.state), '00:24:21');
...
print(legacy.checkpoint, PANEL_ROW + 3, 1, '24M 21S PLAY / 92% COMPLETE');
...
print(worldTotal.checkpoint, PANEL_ROW + 3, 1, '00:24:21 PLAY / 82% COMPLETE');
