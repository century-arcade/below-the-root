...
test("the location marker preserves the last exterior when moving indoors", async () => {
  const { data, entry, at } = await mapFixture();
...
test("crossing empty sky reveals the destination and moves the marker", async () => {
  const { data, at } = await mapFixture();
...
test("replay reconstructs exploration and a new quest resets it", async () => {
