...
O (or the ⚙ button) opens the options: volume (starts at 50%; the slider is squared, so the low end is genuinely quiet; M and the speaker icon mute, which the slider shows as 0%), the CRT effect (a WebGL pass: curvature, scanlines, phosphor stripes, colour bleed; off by default), classic display, the debug tools (the top-bar icons `?debug` shows), and Reset game (asks once, then deletes the autosave and returns to the main menu; DISK STORAGE slots and options stay).  Settings persist in localStorage.
